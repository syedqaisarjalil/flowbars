"""Run bars (volume) — close when the cumulative same-sign volume-run crosses an adaptive threshold.

Same two-component EWMA formula as tick-run bars, but runs are weighted
by trade volume instead of tick count.
"""
...
class RunVolumeBarConstructor(BaseBarConstructor):
    """Bar constructor for volume-run bars.

    A volume-run bar closes when the cumulative same-sign volume-run total
    ...
    initial_ewa_t : float, default 1.0
        Initial seed for :math:`E[T]` (expected run volume per bar).
    ...
    def __init__(
        ...
        accumulator = RunAccumulator(bar_type="run_volume", metric="volume")
    ...
    def from_state(  # type: ignore[override]
        ...
        """Reconstruct a ``RunVolumeBarConstructor`` from a saved state dict."""
        ...
        inst: RunVolumeBarConstructor = BaseBarConstructor.from_state(
...
def compute_run_volume_bars(
    ...
    """Build volume-run bars from a DataFrame of ticks.
...
BarRegistry.register("run_volume", RunVolumeBarConstructor, batch_fn=compute_run_volume_bars)
