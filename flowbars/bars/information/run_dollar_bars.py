"""Run bars (dollar) — close when the cumulative same-sign dollar-run crosses an adaptive threshold.

Same two-component EWMA formula as tick-run bars, but runs are weighted
by notional value (price × volume) instead of tick count.
"""
...
class RunDollarBarConstructor(BaseBarConstructor):
    """Bar constructor for dollar-run bars.

    A dollar-run bar closes when the cumulative same-sign dollar-run total
    ...
    initial_ewa_t : float, default 1.0
        Initial seed for :math:`E[T]` (expected run dollar value per bar).
    ...
    def __init__(
        ...
        accumulator = RunAccumulator(bar_type="run_dollar", metric="dollar")
    ...
    def from_state(  # type: ignore[override]
        ...
        """Reconstruct a ``RunDollarBarConstructor`` from a saved state dict."""
        ...
        inst: RunDollarBarConstructor = BaseBarConstructor.from_state(
...
def compute_run_dollar_bars(
    ...
    """Build dollar-run bars from a DataFrame of ticks.
    ...
    initial_ewa_t : float, default 1.0
        Initial seed for expected run dollar value per bar.
    initial_ewa_proportion : float, default 0.5
...
BarRegistry.register("run_dollar", RunDollarBarConstructor, batch_fn=compute_run_dollar_bars)
