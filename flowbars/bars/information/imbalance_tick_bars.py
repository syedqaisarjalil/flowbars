"""Imbalance bars (tick) — close when absolute signed tick-imbalance crosses an adaptive threshold.

Uses the two-component EWMA formula from AFML §2.4:

.. math::

    T_n = E[T]_n \\times |E[\\theta]_n|

where :math:`E[T]_n` is the expected tick count per bar and
:math:`E[\\theta]_n` is the expected signed-imbalance proportion.
"""
...
class ImbalanceTickBarConstructor(BaseBarConstructor):
    """Bar constructor for tick-imbalance bars.

    A tick-imbalance bar closes when the absolute signed tick-imbalance
    ...
    initial_ewa_t : float, default 1.0
        Initial seed for :math:`E[T]` (expected tick count per bar).
    ...
    def __init__(
        ...
        accumulator = ImbalanceAccumulator(bar_type="imbalance_tick", metric="tick")
    ...
    def from_state(  # type: ignore[override]
        ...
        """Reconstruct an ``ImbalanceTickBarConstructor`` from a saved state dict."""
        ...
        inst: ImbalanceTickBarConstructor = BaseBarConstructor.from_state(
...
def compute_imbalance_tick_bars(
    ...
    """Build tick-imbalance bars from a DataFrame of ticks.
...
BarRegistry.register(
    "imbalance_tick", ImbalanceTickBarConstructor, batch_fn=compute_imbalance_tick_bars
)
